...
Associar um recurso a uma pasta não o torna automaticamente acessível a todos os projetos nela contidos. O administrador da pasta ou do projeto decide quais projetos podem acessar o recurso. Depois de tomar essa decisão, o administrador pode então associar o recurso aos projetos certos.
...
Organização:: Quando você concede a um membro funções de acesso no nível da organização, todas as pastas, projetos e recursos na organização herdam essas funções. Isso significa que o membro tem permissões para tudo na organização.
...
Você pode disponibilizar um Conector para outro projeto nestes casos:
