...
Backup e recuperação do BlueXP:: Faça backup e restauração de dados na nuvem e no local.
...
Cópia e sincronização do BlueXP:: Sincronize dados entre armazenamentos de dados no local e na nuvem.
...
Disposição em camadas do BlueXP:: Estenda seu storage ONTAP no local para a nuvem.
