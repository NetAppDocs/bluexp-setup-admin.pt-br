...
TIP: Se estiver usando o modo padrão ou restrito, você terá uma _organização BlueXP_, que você gerencia usando o gerenciamento de identidade e acesso (IAM) do BlueXP. Mas se você estiver usando o BlueXP  no modo privado, então você terá uma conta _BlueXP _.
...
NOTE: Não é possível editar as credenciais de um perfil de instância associado a uma instância do Connector ou a uma instância do Amazon FSx for ONTAP.
