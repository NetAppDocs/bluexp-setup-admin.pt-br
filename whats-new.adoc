...
A Linha do Tempo agora inclui eventos de login para usuários. Isso permite que você veja quando os usuários efetuaram login no BlueXP, o que pode ajudar na auditoria e no monitoramento de segurança. Usuários da API com a função de administrador da organização podem visualizar o endereço de e-mail do usuário que efetuou login, incluindo o  `includeUserData=true`` parâmetro como no seguinte:  `/audit/<account_id>?includeUserData=true` .
